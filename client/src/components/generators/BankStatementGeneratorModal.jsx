    ...
    const generateStatement = () => {
        ...
        for (let i = 0; i < targetTxnCount; i++) {
            ...
            const getRandomDescription = (isDep) => {
                ...
                // 1. Existing Logic: Randomize if it already has "by" (excluding Self)
                ...
                // 2. NEW Logic: Fix bare descriptions (e.g. "CASH DEPOSIT")
                // Check for bare "Cash Deposit" or "Cash Withdrawal" (ignoring case/whitespace)
                const lowerPick = pick.trim().toLowerCase();
                if (lowerPick === 'cash deposit' || lowerPick === 'cash withdrawal') {
                    // 80% chance of random person, 20% self - giving variety
                    if (Math.random() > 0.2) {
                        const randomName = getRandomNameByCategory(studentCategory);
                        // Maintain case of original string if possible, or default to Title Case
                        const isUpper = pick === pick.toUpperCase();
                        return pick + (isUpper ? ' BY ' : ' by ') + randomName.toUpperCase(); // If original was uppercase, make name uppercase too? Actually user screenshot showed mixed names. Let's stick to standard mixed case for name unless fully UPPER.
                        // Wait, user screenshot showed "CASH DEPOSIT" (all caps).
                        // If "CASH DEPOSIT", maybe we want "CASH DEPOSIT BY [NAME]" or "CASH DEPOSIT by [Name]"?
                        // The user said "Cash Deposit by Ngawang Malla" in the request, which is mixed.
                        // But the screenshot was "CASH DEPOSIT".
                        // Let's use " by [Name]" which is safe.
                    } else {
                        return pick + (pick === pick.toUpperCase() ? ' BY SELF' : ' by Self');
                    }
                }
        ...
        if (generatedTxns.length > 0) {
            ...
            // 1. Distribute the bulk of the gap across ALL transactions to avoid outliers.
            // 2. Use the last transaction for fine-tuning precision.

            // Pass 1: "Rough Smear" to get close
            // IMPROVED LOGIC: Distribute gap based on transaction type to prevent squashing to 100
            let initialSim = runSimulation(generatedTxns);
            let bigGap = targetBal - initialSim.finalBal;

            if (Math.abs(bigGap) > 1000) {
                const deposits = generatedTxns.filter(t => t.isGenerated && t.isDeposit);
                const withdrawals = generatedTxns.filter(t => t.isGenerated && !t.isDeposit);

                if (bigGap > 0) { // Need MORE money
                    // Plan A: Increase Deposits (Safe, no floor)
                    if (deposits.length > 0) {
                        const part = Math.floor((bigGap / deposits.length) / 100) * 100;
                        deposits.forEach(tx => { tx.amount += part; });
                    } else if (withdrawals.length > 0) {
                        // Plan B: Decrease Withdrawals (Must respect minTxn)
                        const part = Math.floor((bigGap / withdrawals.length) / 100) * 100;
                        withdrawals.forEach(tx => {
                            tx.amount -= part;
                            if (tx.amount < minTxn) tx.amount = minTxn;
                        });
                    }
                } else { // Need LESS money (gap is negative)
                    // Plan A: Increase Withdrawals (Safe)
                    if (withdrawals.length > 0) {
                        const part = Math.floor((Math.abs(bigGap) / withdrawals.length) / 100) * 100;
                        withdrawals.forEach(tx => { tx.amount += part; });
                    } else if (deposits.length > 0) {
                        // Plan B: Decrease Deposits (Respect minTxn)
                        const part = Math.floor((Math.abs(bigGap) / deposits.length) / 100) * 100;
                        deposits.forEach(tx => {
                            tx.amount -= part;
                            if (tx.amount < minTxn) tx.amount = minTxn;
                        });
            ...
            // Pass 2: Fine Tuning
            // Find a suitable transaction to adjust (safest to adjust late in period, and one that can handle the gap)
            // Iterate backwards to find a candidate
            ...
            for (let i = 0; i < maxIterations; i++) {
                ...
                // Find candidate: valid index, generated, and won't drop below minTxn
                let candidateIdx = -1;
                for (let idx = generatedTxns.length - 1; idx >= 0; idx--) {
                    if (generatedTxns[idx].isGenerated) {
                        const tx = generatedTxns[idx];
                        // Simulate adjustment
                        let newAmount = tx.amount;
                        if (tx.isDeposit) newAmount += currentGap;
                        else newAmount -= currentGap;

                        // Check constraints (allow epsilon buffer)
                        if (newAmount >= minTxn) {
                            candidateIdx = idx;
                            break;
                        }
                ...
                if (candidateIdx !== -1) {
                    const tx = generatedTxns[candidateIdx];
                    if (tx.isDeposit) tx.amount += currentGap;
                    else tx.amount -= currentGap;
                } else {
                    // Convergence failed with constraints - Force adjust last generated one anyway to prioritize math correctness over realism
                    // Or split gap?
                    // Let's just adjust the last one but clamp to 0 at least.
                    // Ideally this shouldn't happen with Pass 1 doing its job.
                    const lastIdx = generatedTxns.map((t, idx) => t.isGenerated ? idx : -1).lastIndexOf(candidateIdx);
                    // Fallback to strict last generated
                    let fallbackIdx = -1;
                    for (let k = generatedTxns.length - 1; k >= 0; k--) { if (generatedTxns[k].isGenerated) { fallbackIdx = k; break; } }

                    if (fallbackIdx !== -1) {
                        const tx = generatedTxns[fallbackIdx];
                        if (tx.isDeposit) tx.amount += currentGap;
                        else tx.amount -= currentGap;
                        if (tx.amount < 0) tx.amount = 0; // Absolute floor
                    }
                }
        ...
        if (bestResult && Math.abs(bestResult.finalBal - targetBal) < 0.1) {
            bestResult.finalBal = targetBal;
        }
...
<tr><td>Balance of NPR</td><td>:</td><td><b>${formatMoney(totals.balance)}</b></td></tr>
</table>
...
<tr><td>Balance of NPR</td><td>:</td><td><b style="font-size:12pt">${formatMoney(totals.balance)}</b></td></tr>
</table>
                                                ...
                                                <table style={{ width: '100%', marginBottom: '20px', fontSize: '10pt', borderCollapse: 'collapse' }}>
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td style={{ padding: '5px 8px' }}>:</td>
                                                            <td style={{ padding: '5px 0', fontWeight: 'bold', fontSize: '11pt' }}>{formatMoney(totals.balance)}</td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                                    ...
                                                    <div style={{ marginBottom: '24px', fontSize: '9pt', flex: 1 }}>
                                                        ...
                                                        {chunk.map((t, idx) => (
                                                            <div key={idx} style={{ display: 'grid', gridTemplateColumns: '12% 42% 15% 15% 16%', gap: '4px', padding: '3px 0' }}>
                                                                ...
                                                                <div style={{ textAlign: 'right' }}>{t.debit ? formatMoney(parseFloat(t.debit)) : ''}</div>
                                                                <div style={{ textAlign: 'right' }}>{t.credit ? formatMoney(parseFloat(t.credit)) : ''}</div>
                                                                <div style={{ textAlign: 'right' }}>{formatMoney(t.balance)}</div>
                                                            </div>
                                                        ))}
                                                    </div>
                                                            ...
                                                            <div style={{ textAlign: 'center', marginTop: '30px' }}>
                                                                ...
                                                                <div style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', fontSize: '10pt' }}>
                                                                    ...
                                                                    <div style={{ textAlign: 'center' }}>
                                                                        <div style={{ marginBottom: '8px' }}>{formatMoney(totals.debit)}</div>
                                                                        <div>{formatMoney(totals.credit)}</div>
                                                                    </div>
                                                                    <div style={{ textAlign: 'right' }}>
                                                                        <div style={{ fontWeight: '600', marginBottom: '8px' }}>Current Balance</div>
                                                                        <div style={{ fontWeight: 'bold', fontSize: '12pt' }}>{formatMoney(totals.balance)}</div>
                                                                    </div>
                                                                </div>
                                                            </div>
